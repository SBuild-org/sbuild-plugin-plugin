...
The SBuild Plugin Plugin is available from Maven Central Repository under groupId `org.sbuild` and the artifactId `org.sbuild.plugins.sbuildplugin`.

To use it in SBuild 0.7,1 or newer, simply add it to the project classpath.

[source,scala]
----
@classpath("mvn:org.sbuild:org.sbuild.plugin.sbuildplugin:0.2.2")
----
